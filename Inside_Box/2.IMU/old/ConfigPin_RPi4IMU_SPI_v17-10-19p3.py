...
def beginSPI(user_bcm_CSPin, user_bcm_WAKPin, user_bcm_INTPin, user_bcm_RSTPin, user_spiPortSpeed, user_spiPort):  # [2]L33
    ...
    if waitForPinResponse(_int):  # wait for connection of INT Pin
        ...
        spi.open(0, _spiPort)  # first param = 0 is the bus (on RPi only 1 bus = 0), second param = 0* is the
        # device corresponding to the spiPort
...
try:
    ...
    spiPort = 0  # https://www.raspberrypi.org/forums/viewtopic.php?t=59931

    StartIMU(bcm_CSPin, bcm_WAKPin, bcm_INTPin, bcm_RSTPin, spiPortSpeed, spiPort)

    # add callback method on INT pin, bouncetime = time [ms] after an INT that GPIO will ignore new INT
    # https://raspi.tv/2013/how-to-use-interrupts-with-python-on-the-raspberry-pi-and-rpi-gpio-part-2
    # WARNING bouncetime (50ms) = timeBetweenReports/2. Above that value, it won't work
    ...
    while not end_prog:
        ...
        file.write(str(getQuatI()) + ";" + str(getQuatJ()) + ";" + str(getQuatK()) + ";" + str(getQuatReal()) + "\n")
        file.close()
