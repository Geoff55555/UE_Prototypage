...
quat_r_ijk = []
for k in range(4):
    quat_r_ijk.append(0)  # make its length = 4
        ...
        if receiveSPIPacket() == True:
            ...
            try:
                ...
                if (shtpHeader[2] == channel.index("Sensor-report") and shtpData[0] == SHTP_REPORT_BASE_TIMESTAMP):
                    print("[BNO080] has data available from its sensors !")
                    parseInputReport()
                    return True
                elif shtpHeader[2] == channel.index("Control") or shtpHeader[2] == channel.index("Command"):
                    print("[BNO080] has report available about the RPi commands (status, etc.) - NO SENSOR DATA")
...
def parseInputReport():  # [2]L197
    ...
    try:
        ...
        if shtpData[5] == SENSOR_REPORTID_ACCELEROMETER:
            ...
            rawMagY = data2
            rawMagZ = data3
            print("[BNO080] magAcc = ", magAccuracy)
            ...
            quatAccuracy = status
            rawQuatI = data1
            quat_r_ijk[1] = rawQuatI
            rawQuatJ = data2
            quat_r_ijk[2] = rawQuatJ
            rawQuatK = data3
            quat_r_ijk[3] = rawQuatK
            rawQuatReal = data4
            quat_r_ijk[0] = rawQuatReal
            rawQuatRadianAccuracy = data5  # only available on rotation vector, not game rot vect
            # convert quat ? [5] L360
            print("[BNO080] quatAccuracy = ", quatAccuracy)  # here status == quatAccuracy
            print(
                "[BNO080] QUATERNION R-IJK = ", quat_r_ijk)
            if record_file:
...
def parseCommandReport():
    ...
    if shtpData[0] == SHTP_REPORT_COMMAND_RESPONSE:
        ...
        if command == COMMAND_ME_CALIBRATE:
            ...
            print("[BNO080] answered about Save Calibration request")
...
class BNO080:
    ...
    def calibrate(self, is_recording_calibration):
        ...
        if record_file:
            enableDebugRecordFile()  # if we want to record the quaternion data while calibration --> Stability check
    ...
    def update_data(self, user_nbr_of_updates):
    ...
    def continuous_data_update(self):
        # gets data when available
        while dataAvailable():
            self.quat_r_ijk = quat_r_ijk
