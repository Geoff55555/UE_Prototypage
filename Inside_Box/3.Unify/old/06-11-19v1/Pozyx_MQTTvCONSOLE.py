...
def get_id_and_coord(raw_data):
    ...
    if debug_print: print("id set to ", id)
    coord = py_dict["data"]["coordinates"]
    try:
    ...
    if debug_print: print("updated tags arrays (id then coord) :", tags_id, tags_coord)
    # corresponding in tags_id
    data_to_print = 'Tag_ID = ' + id + ' --> Coord = ' + str(coord)
...
def on_message(client, userdata, message):
    ...
    if debug_print: print("message topic =", message.topic)
    if debug_print: print("message qos =", message.qos)
    # handling json
...
class Gateway:
    ...
    def get_coord(self, tag_id_requested):
        try:
            if debug_print: print("tags_id[] = ", tags_id)
            id_tag_requested_index = tags_id.index(tag_id_requested)  # check if this id has been already received
            if debug_print: print("tag id requested = ",tag_id_requested, " found index = " ,id_tag_requested_index)
            return tags_coord[id_tag_requested_index]
        except ValueError:
            print("THE REQUESTED ID TAG :", tag_id_requested, " IS NOT IN THE LIST !")
