...
bool BNO080::beginSPI(int user_CSPin, int user_WAKPin, int user_INTPin, int user_RSTPin, int user_spiPortSpeed, int user_spiPort)
{
	...
	pinMode(_int, INPUT);
	pullUpDnControl(_int, PUD_UP);//???--> https://sourceforge.net/p/raspberry-gpio-python/wiki/Inputs/
	pinMode(_rst, OUTPUT);
	...
	// TODO Clarifier tous les affichages console... Là franchement on ne comprend rien.
	// + mettre des [IMU] pour ne pas mélanger ces messages avec le reste du AHIA
	...
	receivePacket();
	std::cout << "[BNO080] Advertisement message" << std::endl;
	printPacket();
	...
	receivePacket();
	std::cout << "[BNO080] Unsolicited Init" << std::endl;
	printPacket();
	...
	{
		std::cout << "[BNO080] ID REQUEST RESPONSE:" << std::endl;
		printPacket();
		if (shtpData[0] == SHTP_REPORT_PRODUCT_ID_RESPONSE)
		{
			std::cout << "[BNO080] CORRECT ID RESPONSE" <<  std::endl;
			return (true);
		}
	}
