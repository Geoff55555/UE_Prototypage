...
#              [5] IMUManager.cpp modified by Guillaume Villee, [6] BNO08X Datasheet
...
def beginSPI(user_bcm_CSPin, user_bcm_WAKPin, user_bcm_INTPin, user_bcm_RSTPin, user_spiPortSpeed, user_spiPort):  # [2]L33
    ...
    _spiPort = user_spiPort # set to 0 if use of 10 and 11 -MOSI and SCLK- GPIO (SPI0), set to 1 if 20 and 21 (SPI1)
...
def waitForPinResponse(input):
    ...
    time.sleep(.1) # waits 100ms (under that 100ms it won't succeed to connect)
...
def parseInputReport():  # [2]L197
    ...
    try:
        print(dataLength, shtpHeader, shtpData)
        ...
        print("Probably a Package length error : ", e)
...
def checkCalibration():  # [5]L70 + L108 + L311 mix
    ...
    while count < 10:
        ...
        if dataAvailable():  # updates the values if data available (and there will be since calibrateAll() sent calibrate cmd)
            print("quatAcc = ", quatAccuracy, ", magAcc = ", magAccuracy)
            if quatAccuracy == 3 and magAccuracy == 3:  # if True, high accuracy. We need it for both in a row (so 2 following packets received contained 1 quatAcc =3 and the next magAcc = 3 or in opposite order)
                goodData += 1
                print("Good Data = ", goodData, " on ", count, " tests")
            count += 1
        time.sleep(0.5)  # sleeps for 6ms
...
try:
    ...
    bcm_INTPin = 4
    bcm_RSTPin = 18
    spiPortSpeed = 1953000  # 1953000 recommanded for Linux Kernel, max 3000000 for BO080
    spiPort = 0
