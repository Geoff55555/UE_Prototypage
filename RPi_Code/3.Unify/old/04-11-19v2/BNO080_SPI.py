...
def parseInputReport():  # [2]L197
    ...
    try:
        ...
        if shtpData[5] == SENSOR_REPORTID_ACCELEROMETER:
            ...
            print(
                "[BNO080] QUATERNION R-IJK = [", getQuatReal(), " - ", getQuatI(), " , ", getQuatJ(), " , ", getQuatK(),
                " ]")
            if record_file:
...
class BNO080:
    ...
    def get_data(self, user_report_delay_ms):
        # Enable Magnetic Field output -> only magneto
        enableMagnetometer(user_report_delay_ms)  # Send data update every x ms, 100ms has been used for the tests
        # Enable Game Rotation Vector output --> mix accel + gyro
        # enableGameRotationVector(100)  # Send data update every 100ms
        # Enable Rotation Vector
        enableRotationVector(user_report_delay_ms)  # Send data update every x ms, 100ms has been used for the tests
        # gets certain nbr of data updates
        nbr_of_updates = 1
        count = 0
